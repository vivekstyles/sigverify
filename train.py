...
def plot_loss_curves(train_losses, val_losses, save_path):
    ...
    plt.title("Siamese Encoder Training", fontsize=14)
...
def main():
    ...
    val_dataset_eval = SignaturePairDataset(
        DATASET_ROOT, user_ids=VAL_USERS,
        pairs_per_epoch=500, augment=False, seed=999
    )
    ...
    # Save threshold for verification
    torch.save(
        {"threshold": float(eer_threshold), "eer": float(eer)},
        os.path.join(CHECKPOINT_DIR, "threshold.pth")
    )

    # Plot distance distribution
    plot_distance_distribution(
        distances, labels, eer_threshold,
        os.path.join(CHECKPOINT_DIR, "distance_distribution.png")
    )
